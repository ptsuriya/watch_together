...
/*
 * Peek Rating, from React Bits by David Haz — https://reactbits.dev/micro/peek-rating
 * MIT + Commons Clause License Condition v1.0. Copyright (c) 2026 David Haz.
 * Used as part of this application under that license; the component itself is not sold or redistributed.
 * Changed here: the glyphs come from lucide-react, which the app already ships, instead of Hugeicons.
 */
import React, { useEffect, useLayoutEffect, useRef, useState, type CSSProperties, type ReactNode } from 'react';
import { Heart, Star as StarGlyph, Zap, type LucideIcon } from 'lucide-react';

import './peek-rating.css';
...
const SHAPES: Record<PeekRatingShape, LucideIcon> = { star: StarGlyph, heart: Heart, bolt: Zap };
...
const PeekRating: React.FC<PeekRatingProps> = ({
  ...
  // Kept current after each render (not during it), for the window listeners below.
  useEffect(() => {
          ...
          return (
            ...
            >
              ...
              >
                ...
                >
                  {icon ?? <ShapeIcon size={size} fill="currentColor" strokeWidth={1.5} aria-hidden="true" />}
                </span>
              </span>
            </Star>
          );
