...
configs = [(f"W{W} F{F} C{C} d{d}", (W, F, C, d)) for W, F, C, d in [(64, 8, 8, 0.97), (64, 8, 16, 0.97)]]
...
for t in tracks:
    ...
    # The extension's old trick: L-R between 180 Hz and 5.5 kHz, the untouched mix outside it.
    import scipy.signal as ss
    def band(x, lo, hi):
        out = x
        if lo: out = ss.sosfilt(ss.butter(2, lo, "highpass", fs=44100, output="sos"), out)
        if hi: out = ss.sosfilt(ss.butter(2, hi, "lowpass", fs=44100, output="sos"), out)
        return out
    def classic(x):
        side = band(1.4 * (x[0] - x[1]), 180, 5500)
        keep = [band(c, None, 180) + band(c, 5500, None) for c in x]
        return np.asarray([side + keep[0], side + keep[1]])
    lr.append((sdr(acc, classic(mix)), 10 * np.log10((classic(voc) ** 2).sum() / (voc ** 2).sum())))
...
print(f"{len(tracks)} tracks; doing nothing scores {np.mean(base):.2f} dB; classic L-R {np.mean([a for a, _ in lr]):.2f} dB, vocal left {np.mean([b for _, b in lr]):.1f} dB")
