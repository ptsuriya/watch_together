...
The host decides who may change the key: only the host, only whoever queued the song, or anyone in the room. The two karaoke modes differ in where the sound comes from: on one screen the host's machine plays for the room, so only that machine needs the key extension; in karaoke together every device plays for itself, so each device shifts its own sound and says whether it can.
...
- **Tournament** — a knock-out for the whole room, in one of two shapes. **แข่งเพลงเดี่ยว**: everyone still in sings one song a round and the round's lowest score goes home. **แบทเทิลจับคู่**: the room is drawn into pairs, both sing, the lower score of each pair goes home and the winners meet in the next round (an odd player gets a bye). The draw comes from chance, from the night's table (best against worst), or from the players themselves — **เลือกตำแหน่งเอง** randomises the order of choosing and then each person picks their own slot from their phone, so the last to choose takes what is left. Each round can be given a genre (แร็พ, ลูกทุ่ง, …) that every screen shows. Starting it turns scoring on and puts the crossfade aside — a performance should end, not melt into the next singer's — and stopping gives the crossfade back. Every screen shows the round and who still owes a song, and the moments themselves (the start, each knock-out, the champion) come up big for a few seconds on the shared screen and on everyone's own.
...
Pitch-shifting YouTube audio is only possible from inside the embed, so the key buttons need the **KUMA Karaoke Key** extension on the screen making the sound (a computer running Chrome, Edge, Opera or another Chromium browser; Firefox has steps too, untested). Its source is in `extension/`, and `npm run extension:zip` (also run by `npm run build`) packs `public/kuma-karaoke-key.zip`, which the karaoke sidebar offers for download.

Who gets told about it: a karaoke host meets the setup dialog the moment the room opens, because their machine is the only one making sound. In karaoke together every screen shifts for itself, so the dialog opens once on any computer that could install it and has not, while phones and TVs get a line under the key instead — they cannot install anything, and saying so beats a dialog with nothing to press.

Install while it is not in a store yet: download the zip, unzip it, open the browser's extensions page, turn on developer mode and load the unpacked folder. The setup dialog reads the browser and prints its own address — `chrome://extensions`, `edge://extensions`, `opera://extensions` — with that browser's button names. Firefox instead gets `about:debugging#/runtime/this-firefox` and **Load Temporary Add-on**, which takes the zip as it is but forgets it when Firefox closes; those steps are offered, not verified. Set `NEXT_PUBLIC_KARAOKE_EXTENSION_URL` once the extension is published, and the sidebar shows a one-click store button instead.

The same extension can also thin the guide vocal (1.2.0 and up). **ลดเสียงร้องต้นฉบับ** in the room settings asks it to subtract the centre channel — all of it, or half — which is the old L−R karaoke trick: whatever sits dead centre in the mix cancels out. Measured on a synthetic mix, a centred 1 kHz "voice" drops to zero while a left-only 200 Hz "guitar" survives; on real songs it also takes the centred bass and kick with it, so only the band a voice sings in is cancelled: below 180 Hz and above 5.5 kHz the untouched mix comes through. Measured on a synthetic mix of centred parts, a 60 Hz kick and a 9 kHz hi-hat come back to full while a 1 kHz voice drops to 0.02; a centred clap keeps its snap and loses the body it had inside the vocal band, which is the trick working, not failing. It is still mono, and a voice that was not mixed centre barely changes, which is why half is offered too. **โทนเสียง** under it (1.3.0 and up) is one tap on a preset — ปกติ, เบสแน่น, ร็อค, ป็อป, ร้องชัด (the original singer forward, for learning a song) or ดนตรีเด่น (the guide vocal tucked behind the band, for singing over it without the mono cost of the cut) — with a five-band EQ folded under it (90 Hz shelf, 250 Hz, 1 kHz, 3 kHz, 8 kHz shelf, ±8 dB). Whenever the extension processes the audio at all, a limiter just under full scale sits last, and boosts past 4 dB lower the whole EQ by the rest: rendered offline against a 0.95 sine, the presets that used to peak at up to 1.58 and all bands at +8 at 2.88 (hard clipping) now stay under 1.03. The idea of presets over sliders came from looking at NextSona, a Chrome extension that runs a UVR vocal-separation model in the browser; its code is not ours to reuse and none of it is here, and why the room does not do AI separation yet is in docs/handoff.md. An extension loaded from a folder never updates itself — only a store listing can do that — so the room watches the version the extension reports and says when it is behind: the key card shows a line, the setup dialog names the installed version and offers the new zip, and the steps say to drop it over the old folder and press reload. Auto-updates would mean publishing: Edge Add-ons is free, the Chrome Web Store is a one-time $5.

**AI vocal removal (1.4.0).** Next to the vocal select sits an **AI** switch, on by default: each screen that plays the song takes the voice out with a neural network on its own GPU (tsurumeso's vocal-remover v4, MIT, run through ONNX Runtime Web on WebGPU) instead of L−R. It keeps the stereo and the band, and on 12 MUSDB18 test excerpts it scores 11.0 dB accompaniment SDR with the voice 8 dB down, where L−R barely moves those voices at all. It needs half a second of the song ahead of what it decides, so that screen plays sound and picture 0.58 s late together — the extension draws the video on a canvas over the embed, held back by the same amount, so burned-in lyrics stay on the beat — and in a sing-along the room makes that delay up to the other screens (the host moves the room through `hostOffset`, a guest runs its own copy ahead). A screen without WebGPU, or with a GPU too slow to keep up (the worker times it before promising), says so under the control and uses L−R. Ads skip the network and the delay, like every other effect. How it is built and measured is in `extension/README.md`.

How it works: the extension's content script runs in the `youtube-nocookie.com` embed, and the room posts `{source: "kuma-listening-party", type: "key", semitones}` to that frame. The script routes the video through Web Audio into [Signalsmith Stretch](https://signalsmith-audio.co.uk/code/stretch/) (MIT, vendored in `extension/vendor/`), which shifts the pitch with about 70 ms of latency. Two buttons move the key, half a semitone per press; the room counts presses, so `+3` on screen is 1.5 semitones in the extension. At key 0 the audio is not processed at all, and while YouTube shows an ad the pitch shift steps aside so the ad plays exactly as YouTube sent it. Nothing is collected or sent anywhere; the extension asks for no permissions beyond that one site.
